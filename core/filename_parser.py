...
def parse_filename1(filepath: str) -> ProjectInfo:
    """
    Parse PLY filename to extract project information.
    
    Expected format: .../Projects/{project}/{job}/{job}#{yyyMMdd_hhmmss}#{segment}.ply
    ...
    # Try to parse filename with # delimiter
    parts = name_without_ext.split('#')
    
    if len(parts) >= 3:
        # Expected: job#yyyMMdd_hhmmss#segment
        parsed_job = parts[0]
        scan_time = parts[1]
        segment_name = '#'.join(parts[2:]) if len(parts) > 2 else ""
        
        # Use directory names if they match
