"""
NoteTool: click the cloud to place a 3D-anchored text note (opens an
editor dialog), drag an existing note's label to reposition it, double-click
a label to edit it, Delete to remove the selected one.
"""
...
NoteEditor = Callable[[Optional[NoteAnnotation]], Optional[dict]]

DRAG_THRESHOLD_PX = 3


def _default_note_editor(existing: Optional[NoteAnnotation]) -> Optional[dict]:
...
class NoteTool(Tool):
    ...
    def status_hint(self) -> str:
        return (
            "Click cloud: add note  |  Drag label: move  |  Double-click: edit  |  "
    ...
    def _draw_drag_preview(self, note: NoteAnnotation) -> None:
        ...
        self.ctx.overlay.set_hud_text(f"Moving note: {note.text[:40]!r}")
        self.ctx.request_render()
    ...
    # ------------------------------------------------------------------ create / edit
    def _create_note(self, anchor) -> None:
        result = self.note_editor(None)
        if result is None:
            return
    ...
    def _edit_note(self, note: NoteAnnotation) -> None:
        result = self.note_editor(note)
        if result is None:
            return
