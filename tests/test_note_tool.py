"""
Tests NoteTool logic directly via synthetic PointerEvent/KeyEvent objects.
...
from pps.tools import note as note_module
...
def fake_geometry(monkeypatch):
    ...
    monkeypatch.setattr(
        note_module, "project_to_screen", lambda pts, plotter: np.array([[p[0], p[1]] for p in pts])
    )
    ...
    monkeypatch.setattr(note_module, "compute_label_bbox", fake_bbox)
...
def test_click_creates_note(document, monkeypatch):
    monkeypatch.setattr(note_module, "pick_nearest_point_3d", lambda x, y, plotter: (0.0, 0.0, 0.0))
    editor_calls = []

    def fake_editor(existing):
        editor_calls.append(existing)
        return {"text": "hello", "color": "#ff0000", "font_size": 20}

    tool = NoteTool(note_editor=fake_editor)
...
def test_click_with_editor_cancelled_creates_nothing(document, monkeypatch):
    monkeypatch.setattr(note_module, "pick_nearest_point_3d", lambda x, y, plotter: (0.0, 0.0, 0.0))
    tool = NoteTool(note_editor=lambda existing: None)
    tool.activate(make_ctx(document))
...
def test_click_on_empty_space_creates_nothing(document, monkeypatch):
    monkeypatch.setattr(note_module, "pick_nearest_point_3d", lambda x, y, plotter: None)
    called = []
    tool = NoteTool(note_editor=lambda existing: called.append(existing) or {"text": "x"})
    tool.activate(make_ctx(document))
...
def test_press_on_label_selects_without_opening_editor(document):
    ...
    editor_calls = []
    tool = NoteTool(note_editor=lambda existing: editor_calls.append(existing))
    tool.activate(make_ctx(document))
